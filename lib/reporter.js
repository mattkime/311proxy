...
if( process.env.NODE_ENV == 'dev'){
	...
	return {
		...
		let justTheFinalForm = function (text ){
			let lessText = justTheForm( text );
			if( lessText.indexOf('_finish') !== -1){
				let document = jsdom.jsdom( text ),
					rows = document.querySelectorAll('td.label'),
					arr = Array.prototype.slice.call(rows);

				arr.map(function(item){ return item.parentElement})
					.forEach(function(item){
						let labelEl = item.querySelector('.label'),
							fieldEl = item.querySelector('.field div'),
							labelStr = labelEl ? labelEl.innerHTML : null,
							fieldStr = fieldEl ? fieldEl.innerHTML : null;

						console.log( `${labelStr} ${fieldStr}` );
					});
				console.log('*****************')
			}
		};
